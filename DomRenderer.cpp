...
namespace SnooDom
{
    ...
    {
        dom_builder_state(SimpleSessionMemoryPool^ pool) : domId(1), memoryPool(pool) {}
        uint32_t domId;
    ...
    Document::Document(SimpleSessionMemoryPool^ memoryPool) : IDomContainer(std::vector<IDomObject*>(), 0), State(new dom_builder_state(memoryPool)) {}
    Document::~Document() {}


#ifdef _WINRT_DLL
    SnooDom::SnooDom(SimpleSessionMemoryPool^ memoryPool) : document(new Document(memoryPool)) {}
    ...
    SnooDom^ SnooDom::MarkdownToDOM(Platform::String^ source, SimpleSessionMemoryPool^ memoryPool)
    {
        try
        {
            //when this goes out of scope, all memory consumed by this session will be freed
            auto result = ref new SnooDom(memoryPool);
            if (source == nullptr)
            {
                return result;
            }
